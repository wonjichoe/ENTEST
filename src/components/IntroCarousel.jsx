...
export function IntroCarousel() {
  ...
  return (
    <section className="relative h-[400px] sm:h-[600px] md:h-[746px] overflow-hidden">
      <Carousel 
        ...
        {slides.map((slide, index) => (
          <div key={index} className="relative h-[400px] sm:h-[600px] md:h-[746px]">
            ...
            <div className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center">
              <div className="w-full max-w-[375px] sm:max-w-[810px] lg:max-w-[1440px] mx-auto text-left">
                <p className="text-white text-2xl sm:text-4xl md:text-6xl md:leading-[1.4] font-bold mb-4 sm:mb-8 md:mb-14 max-w-full sm:max-w-[80%] md:max-w-[60%] break-keep">
                  {slide.caption}
                </p>
      ...
      <button
        onClick={prev}
        className="absolute left-2 sm:left-4 top-1/2 transform -translate-y-1/2 text-white text-4xl sm:text-5xl md:text-6xl"
      >
        <IoIosArrowBack />
      </button>
      <button
        onClick={next}
        className="absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2 text-white text-4xl sm:text-5xl md:text-6xl"
      >
        <IoIosArrowForward />
      </button>
    </section>
